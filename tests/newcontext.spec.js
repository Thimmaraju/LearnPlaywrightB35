...
test.describe('Automation - Working With Elements', () => {
  ...
  test("browse context test", async ({ browser }) => {
    ...
    await page1.getByPlaceholder('First Name').fill(`sangeetha`);
    await page1.getByPlaceholder('Last Name').fill(`g`);
    await page1.locator('//input[@class="oxd-input oxd-input--active"]').last().fill(`43y6578`);
    await page1.locator('//input[@type="checkbox"]').dispatchEvent('click');
    await page1.locator("//label[text()='Username']/../following-sibling::div/input").fill("sangeethagg")
    ...
    await page2.locator('input[name="username"]').fill("sangeethag")
